...
def user_input():
    user_temp = round(random.uniform(20.0, 50.0),2)
    user_humid =  round(random.uniform(20.0, 60.0),2)
    return user_temp, user_humid
...
def temp_sensor_input():
    ...
    if ((received_data.find("Gesture"))>=0):
        ...
        client.publish("HDU/Building1/Monitoring/Emergency", 1)
    ...
    if (received_data.find("Temperature")>=0):
        ...
        if ((received_data1.find("Humidity"))>=0):
            ...
            client.publish("HDU/Building1/Monitoring/Humidity", sensor_humidity1)
...
def temp_planning():
    ...
    set_temp, set_humid = user_input()
    print(set_temp, set_humid)

    if (float(sensed_temp ) == set_temp or float(sensed_humidity) == set_humid):
        ...
        return data
    elif (set_temp > float(sensed_temp ) or set_humid > float(sensed_humidity)):
...
#python new_planning.py domain.pddl problem.pddl plan.txt